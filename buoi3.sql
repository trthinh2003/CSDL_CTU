...
select count(*) so_tacpham_trong_thuvien
from tacpham;
...
select count(*)
from (select tacgia, count(*) sotp from tacpham group by tacgia having count(*) >= 2);

--24) Tìm t?a tác ph?m có nhi?u ng??i m??n nh?t
select m.ns, tp.tua, count(*) so_lan_muon
from tacpham tp
join sach s on tp.nt = s.nt
join muon m on s.ns = m.ns
group by m.ns, tp.tua
...
select m.nd, d.ho, d.ten, count(*) solanmuon 
from muon m
join sach s on s.ns = m.ns
join tacpham tp on tp.nt = s.nt
join docgia d on m.nd = d.nd
group by m.nd, d.ho, d.ten
having count(*) = (select max(count(distinct tpl.nt)) from muon ml join sach sl on sl.ns = ml.ns join tacpham tpl on tpl.nt = sl.nt group by ml.nd);
...
join docgia d on m.nd = d.nd
group by m.nd, d.ten
having count(*) = (select min(count(distinct tpl.nt)) from muon ml join sach sl on sl.ns = ml.ns join tacpham tpl on tpl.nt = sl.nt group by ml.nd);
...
select tua
from tacpham
where nt not in (select nt from sach s left join muon m on s.ns = m.ns where ngaymuon is null);
